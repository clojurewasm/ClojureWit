...
(def VERSION
  "Bumped when emission changes, so a `:modified` verdict from
   `cljwit.project/status` can say \"newer generator\" instead of blaming
   the component (`0021` B)."
  "1")

(def ^:private KNOWN-OPTS #{:ns :dir :interface :rename :engine :source :regenerate})

(defn- known-opts!
  "An unknown option silently dropped is `0014`'s nil-on-typo — a review
   caught :renmae sailing through this very function's callers."
  [opts]
  (when-let [bad (seq (remove KNOWN-OPTS (keys opts)))]
    (throw (ex-info (str "unknown option(s): " (pr-str (vec bad)))
                    {:cljwit/error :bad-options :cljwit/keys (vec bad)}))))
...
(defn source-for
  ...
  (let [rows     (plan described opts)
    ...
    (str ";; Generated by cljwit.host.gen (doc/design/0020). Do not edit by hand.\n"
         ...
         ";; generator: cljwit.host.gen v" VERSION "\n"
         ";; regenerate: " (or (:regenerate opts)
                               (str "(cljwit.host.gen/write-ns! \""
                                    (or source "<component.wasm>") "\" '" regen ")")) "\n"
         "(ns " ns "\n"
